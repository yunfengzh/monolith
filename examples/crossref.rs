...
impl Rhai {
    ...
    fn eval_script(&mut self) {
        ...
        self.engine
            .register_fn("declare_trait", Rhai::declare_trait);
    ...
    fn call<T: Clone + 'static + Send + Sync>(
        ...
        self.engine
            .call_fn_with_options(options, &mut self.scope, &self.ast, fn_name, args)
    }
}
// }])>
    ...
    fn adjust(&mut self, value: i64) {
        ...
        if self.life <= 0 {
            for i in self.consumers.iter() {
                let ret = i.on_player_die(
                    Hurt {
                        critical_attack: value,
                        state: "need heal".to_string(),
                    },
                    17,
                );
...
impl PlayerProxy {
    fn proxy(rhai: &mut Rhai, player: &mut Player) {
        ...
        rhai.engine
            .register_fn("get_helmet", PlayerProxy::get_helmet);
...
impl Life for LifeToRhai {
    fn on_player_die(&self, evt: Hurt, unused: i64) -> Revive {
        ...
        let ret: Dynamic = rhai_call
            .call_method(obj, "on_player_die", (evt, unused))
            .unwrap();
